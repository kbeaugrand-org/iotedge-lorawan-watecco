...
from .WattecoStandardReportDecoderModule.mai
from .Batch import br_uncompress
from .Batch.br_uncompress import hex_to_array
from .Codec.Decoding_Functions import Decoding_JSON, version
...
def parseFor(tagz: int, commands, payload: str):
    ...
    if ((hex_to_array(inputData)[0] % 2) == 0): 
        ...
        result = {}
        result["originalPayload"] = inputData  
        result["data"] = []

        for x in batch["dataset"]: 
            item = {}
            item["timestamp"] = x["data_absolute_timestamp"]
            item["version"] = version
            item["name"] = x["data"]["label_name"]
            item["type"] = ""
            item["value"] = x["data"]["value"]
...
def parseUnitaryFrame(payload: str): 
    ...
    item["version"] = version
    item["name"] = frame["AttributeID"]
    item["type"] = frame["AttributeType"]
    item["value"] = frame["Data"]
